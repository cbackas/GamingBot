...
public class CommandGameNight implements Command {
    @Override
    ...
    public void execute(GamingBot bot, IDiscordClient client, String[] args, IGuild guild, IMessage message, boolean isPrivate) {
        ...
        if (roles.contains(guild.getRoleByID(GamingRoles.ADMIN.id)) || roles.contains(guild.getRoleByID(GamingRoles.MOVIENIGHT.id))) {
            ...
            if (matcherOption.find()) {
                ...
                if (option.equalsIgnoreCase("set")) {
                    ...
                    if (matcherSet.find()) {
                        ...
                        IMessage thingy = Util.sendBufferedMessage(guild.getChannelByID(GamingBot.ANNOUNCEMENT_CHANNEL_ID), announcement);
                        String messageID = thingy.getID();

                        Util.sendBufferedMessage(guild.getChannelByID(GamingBot.GENERAL_CHANNEL_ID), announcement);
                        try {
                    ...
                    if (matcherAnnounce.find()) {
                        ...
                        IMessage message2Delete = Util.sendBufferedMessage(guild.getChannelByID(GamingBot.ANNOUNCEMENT_CHANNEL_ID), announcement);
