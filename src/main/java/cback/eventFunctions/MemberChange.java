...
public class MemberChange {
    ...
    public MemberChange(GamingBot bot) {
        this.bot = bot;
    }
    ...
    public void memberLeave(UserLeaveEvent event) {
        ...
        //Mute Check
        if (bot.getConfigManager().getConfigArray("muted").contains(event.getUser().getID())) {
            Util.sendMessage(event.getGuild().getChannelByID("263185438380916737"), user + " is muted and left the server. Their mute will be applied again when/if they return.");
        }
